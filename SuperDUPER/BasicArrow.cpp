...
std::unique_ptr<Projectile> BasicArrow::clone(Angle facingDirection, Position<float> position, int ownerId) const {
	return std::unique_ptr<Projectile> {new BasicArrow{ facingDirection, position, getSpeed(), getRange(),
